...
def SNRtest(clean_speech, processed_speech,fs):
    eps=np.finfo(np.float64).eps
    signal_energy = np.power(0.02*clean_speech,2).sum(-1)
    noise_energy = np.power(clean_speech-processed_speech,2).sum(-1)
    snr = 10*np.log10(signal_energy/(noise_energy+eps)+eps)
    return snr
...
def pesq(clean_speech, processed_speech, fs):
    if fs == 8000:
        ...
        # mos_lqo = pypesq.pesq(clean_speech, processed_speech, fs, 'wb')
